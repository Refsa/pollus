    ...
    where TQueryFilters : ITuple, IFilter, new()
{
    ...
    readonly struct UpdateJob : IForEach<Read<Transform2D>, Read<CollisionShape>>
    {
        public readonly SpatialQuery SpatialQuery { get; init; }
        public readonly Vec2f Offset { get; init; }

        public readonly void Execute(scoped in Entity entity, scoped ref Read<Transform2D> transform, scoped ref Read<CollisionShape> shape)
        {
    ...
    readonly struct UpdateWithLayerJob : IForEach<Read<Transform2D>, Read<CollisionShape>, Read<Layer>>
    {
        public readonly SpatialQuery SpatialQuery { get; init; }
        public readonly Vec2f Offset { get; init; }

        public readonly void Execute(scoped in Entity entity, scoped ref Read<Transform2D> transform, scoped ref Read<CollisionShape> shape, scoped ref Read<Layer> layer)
        {
            var boundingCircle = shape.Component.GetBoundingCircle(transform.Component);
