...
public unsafe interface IWgpuBackend : IDisposable
{
    ...
    NativeHandle<BufferTag> DeviceCreateBuffer(NativeHandle<DeviceTag> device, in BufferDescriptor descriptor, Utf8Name label);
    void BufferDestroy(NativeHandle<BufferTag> buffer);
    void BufferRelease(NativeHandle<BufferTag> buffer);
    void QueueWriteBuffer(NativeHandle<QueueTag> queue, NativeHandle<BufferTag> buffer, nuint offset, void* data, nuint size);
    NativeHandle<TextureTag> DeviceCreateTexture(NativeHandle<DeviceTag> device, in TextureDescriptor descriptor, Utf8Name label);
    void TextureDestroy(NativeHandle<TextureTag> texture);
    void TextureRelease(NativeHandle<TextureTag> texture);
    void QueueWriteTexture(NativeHandle<QueueTag> queue, NativeHandle<TextureTag> texture, uint mipLevel, uint originX, uint originY, uint originZ, void* data, nuint dataSize, uint bytesPerRow, uint rowsPerImage, uint writeWidth, uint writeHeight, uint writeDepth);
    NativeHandle<SamplerTag> DeviceCreateSampler(NativeHandle<DeviceTag> device, in SamplerDescriptor descriptor, Utf8Name label);
    void SamplerRelease(NativeHandle<SamplerTag> sampler);
    NativeHandle<BindGroupLayoutTag> DeviceCreateBindGroupLayout(NativeHandle<DeviceTag> device, in BindGroupLayoutDescriptor descriptor, Utf8Name label);
    void BindGroupLayoutRelease(NativeHandle<BindGroupLayoutTag> layout);
    NativeHandle<BindGroupTag> DeviceCreateBindGroup(NativeHandle<DeviceTag> device, in BindGroupDescriptor descriptor, Utf8Name label);
    void BindGroupRelease(NativeHandle<BindGroupTag> bindGroup);
    NativeHandle<CommandEncoderTag> DeviceCreateCommandEncoder(NativeHandle<DeviceTag> device, Utf8Name label);
    void CommandEncoderRelease(NativeHandle<CommandEncoderTag> encoder);
    NativeHandle<CommandBufferTag> CommandEncoderFinish(NativeHandle<CommandEncoderTag> encoder, Utf8Name label);
    ...
    NativeHandle<ShaderModuleTag> DeviceCreateShaderModule(NativeHandle<DeviceTag> device, ShaderBackend backend, Utf8Name label, Utf8Name code);
    void ShaderModuleRelease(NativeHandle<ShaderModuleTag> shaderModule);
    NativeHandle<PipelineLayoutTag> DeviceCreatePipelineLayout(NativeHandle<DeviceTag> device, in PipelineLayoutDescriptor descriptor, Utf8Name label);
    void PipelineLayoutRelease(NativeHandle<PipelineLayoutTag> layout);
    NativeHandle<ComputePipelineTag> DeviceCreateComputePipeline(NativeHandle<DeviceTag> device, in ComputePipelineDescriptor descriptor, Utf8Name label);
    void ComputePipelineRelease(NativeHandle<ComputePipelineTag> pipeline);
    NativeHandle<TextureViewTag> TextureCreateView(NativeHandle<TextureTag> texture, in TextureViewDescriptor descriptor, Utf8Name label);
    void TextureViewRelease(NativeHandle<TextureViewTag> view);
    NativeHandle<RenderPipelineTag> DeviceCreateRenderPipeline(NativeHandle<DeviceTag> device, in RenderPipelineDescriptor descriptor, Utf8Name label);
    ...
    void RenderPassEncoderSetBindGroup(NativeHandle<RenderPassEncoderTag> pass, uint groupIndex, NativeHandle<BindGroupTag> bindGroup, uint dynamicOffsetCount, uint* dynamicOffsets);
    ...
    NativeHandle<ComputePassEncoderTag> CommandEncoderBeginComputePass(NativeHandle<CommandEncoderTag> encoder, Utf8Name label);
    ...
    void ComputePassEncoderSetBindGroup(NativeHandle<ComputePassEncoderTag> pass, uint groupIndex, NativeHandle<BindGroupTag> bindGroup, uint dynamicOffsetCount, uint* dynamicOffsets);
